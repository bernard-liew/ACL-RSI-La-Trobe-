...
node_col <- ifelse (names (res$data[[1]]) %in% sf, "blue", "white")
```
...
The networks between the 6th and 12th month are not significantly different (p = `r pair_res$nwinv.pval`). 
...
The networks are not significantly different (p = `r pair_res$nwinv.pval`). 
